...
describe("DashboardEngine", () => {
    it("loads layout, filters and widgets in configured order", () => {
        ...
        expect(result.dashboard.filters?.map(filter => filter.field)).toEqual([
            "Item_Desc", "Br_code", "Cat_code", "Std_Vat",
        ]);
    ...
    it("maps inline table and stat SQL resources", () => {
        ...
        const sharedFilters = result.dashboard.filters ?? [];
        expect(resolveDashboardWidgetRequest(table, sharedFilters)).toMatchObject({
            action: "sql",
        ...
        expect(resolveDashboardWidgetRequest(stat, sharedFilters)).toMatchObject({
            action: "sql",
    ...
    it("resolves every inline Bill widget through the shared backend SQL resource mode", () => {
        ...
        const resources = result.dashboard.widgets.map(widget => {
            ...
            const request = resolveDashboardWidgetRequest(widget, result.dashboard.filters ?? []);
    ...
    it("supports inline report widgets", () => {
        const inlineReport = {
            id: "inline-report",
            type: "report",
            title: "Inline report",
    ...
    it("preserves widgetId validation for reusable definitions", () => {
        const result = validateDashboard({
            id: "reusable-dashboard",
            title: "Reusable dashboard",
            widgets: [{
                id: "shared-stat-instance",
                type: "stat",
                title: "Shared statistic",
                widgetId: "shared-stat-definition",
                valueField: "Total",
            }],
        }, { widgetIds: ["shared-stat-definition"] });
